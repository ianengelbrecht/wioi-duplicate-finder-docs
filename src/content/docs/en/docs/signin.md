...
In rare cases where the local SQLite database fails to open or gets corrupted (for example, due to an unexpected computer shutdown), the application displays a **Database Recovery** page.

This interface provides two options:
- **Retry Check**: Attempts to re-initialize and repair the database connection.
- **Restore Backup**: Allows you to browse your local computer and restore the database state from a previously exported database backup file (`.db`).
